'use strict';
// Forked from https://github.com/kaimallea/node-imgur, MIT License
...
imgur._imgurRequest = function(operation, payload, extraFormParams) {
  ...
  if (!operation || typeof operation !== 'string' || (!payload && operation !== 'credits')) {
    deferred.reject(new Error('Invalid argument', !operation, typeof operation !== 'string', !payload));
  ...
  switch (operation) {
    ...
    case 'createAlbum':
      options.method = 'POST';
      options.uri += 'album';
      if (payload.title || payload.description || payload.layout || payload.privacy) {
        options.uri += '?';
      }

      if (payload.title && payload.description) {
        options.uri += 'title=' + encodeURIComponent(payload.title);
        options.uri += '&description=' + encodeURIComponent(payload.description);
      }

      // if (payload.layout) {
      //   options.uri += 'layout=' + payload.layout;
      // }
      //
      // if (payload.privacy) {
      //   options.uri += 'privacy=' + payload.privacy;
      // }
 ...
 * @returns {promise}
 */
imgur.createAlbum = function(options) {
  var deferred = Q.defer();

  if (!options) {
    options = 'dummy'; // What a hack...
  }
  console.log("createAlbum options", options);

  imgur._imgurRequest('createAlbum', options)
      .then(function(json) {
        deferred.resolve(json);
      })
      .catch(function(err) {
 ...
 * @param   {Object} options - options to pass to createAlbum(), such as title and description
 ...
 */
imgur.uploadAlbum = function(images, uploadType, options, failSafe) {
  var deferred = Q.defer();
