  ...
  function updateScores() {
    ...
    request(options).then((res) => {
      ...
      for (let user of adapter.users) {
        ...
        if (['adamslu@charter.net', 'anuheik1@gmail.com', 'mmmbarkley@gmail.com'].indexOf(user.email) > -1) {
          continue;
        }
        if (!user.custom) {
      ...
      let noTeamMessage = new PushMessage(new User({}), pushTitle,
        'Set a team name to track your score!', adapter);
      let badTeamMessage = new PushMessage(new User({}), pushTitle,
        'Your team name didn\'t match any scores. Please check it!', adapter);
      ...
      for (let teamUser of teamUsers) {
        ...
        let msg = new PushMessage(new User({}), pushTitle,
          `${teamUser.custom.team_name} is in ${place} with ${score.score} points!`, adapter);
